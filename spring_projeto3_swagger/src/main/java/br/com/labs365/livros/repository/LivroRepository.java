package br.com.labs365.livros.repository;
...
public class LivroRepository {
    ...
    public LivroRepository() {
        List<Livro> cadastrados = List.of(
                new Livro(++idCounter, "Philosophiae Naturalis Principia Mathematica", "14632196816", 1713, "Isaac Newton"),
    ...
    public Livro findById(Integer id) {
        return livros.stream().filter(l -> l.getId().
                        equals(id))
                .findFirst().
    ...
    public Livro update(Livro alterado) {
        boolean isExcluido = livros.removeIf(l -> l.getId().
                equals(alterado.getId()));
        if (!isExcluido)
